/**
 * RunInfra Provider Extension
 *
 * Registers RunInfra as a custom provider using the openai-completions API.
...
interface JsonModel {
  ...
  compat?: {
    ...
    thinkingFormat?: "openai" | "deepseek" | "zai" | "qwen" | "qwen-chat-template" | "chat-template";
...
/**
 * Transform a model from the RunInfra /v1/models API.
 * RunInfra already reports pricing in per-million-token USD and exposes
 * context_window / max_output_tokens / cached_input_price_usd_per_mtok.
 */
function transformApiModel(apiModel: any): JsonModel | null {
  ...
  const model: JsonModel = {
    ...
    cost: {
      ...
      cacheRead: typeof apiModel.cached_input_price_usd_per_mtok === "number" ? apiModel.cached_input_price_usd_per_mtok : 0,
      cacheWrite: 0,
    },
...
export default function (pi: ExtensionAPI) {
  ...
  // Attach a fresh X-Client-Request-Id UUID to every RunInfra request.
  // RunInfra uses this header for request tracing; a unique value per call
  // keeps server-side logs and idempotency scoped to a single attempt.
